...
void Viewer::ValidateDimensionsForViewing(roadmanager::RMObject& object) const
{
    ...
    {
        LOG("Object %s missing length, set to %.2f for viewer purpose", object.GetName().c_str(), DEFAULT_MIN_DIM);
    }
    if (IsEqualDouble(object.GetWidth().Get(), 0.0))
    {
        LOG("Object %s missing width, set to %.2f for viewer purpose", object.GetName().c_str(), DEFAULT_MIN_DIM);
    }
    if (IsEqualDouble(object.GetHeight().Get(), 0.0))
    {
        LOG("Object %s missing height, set to %.2f for viewer purpose", object.GetName().c_str(), DEFAULT_MIN_DIM);
    }
}
