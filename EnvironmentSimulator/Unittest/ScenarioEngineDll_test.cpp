...
TEST(GetOSIRoadLaneTest, lane_no_obj)
{
    ...
    SE_FlushOSIFile();
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 83621);  // initial OSI size, including static content
    ...
    SE_FlushOSIFile();
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 84510);  // slight growth due to only dynamic updates

    SE_StepDT(0.001f);  // Step for write another frame to osi file
    SE_UpdateOSIGroundTruth();
    SE_FlushOSIFile();
    ASSERT_EQ(stat("gt.osi", &fileStatus), 0);
    EXPECT_EQ(fileStatus.st_size, 85400);  // slight growth due to only dynamic updates
